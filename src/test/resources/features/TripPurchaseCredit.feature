...
#  Scenario: Отправка формы с пустым значением в поле "Номер карты"
#    When пользователь заполняет форму, не указав номер карты
#    And нажимает кнопку "Продолжить"
#    Then появляется сообщение об обязательности поля для заполнения
  ...
  Scenario: Отправка формы с отсутствующей фамилией в поле "Владелец"
  ...
  Scenario Outline: Отправка формы с нелатинскими буквами в поле "Владелец"
    When пользователь заполняет форму, указав имя и фамилию нелатиницей <locale> в поле "Владелец"
    And нажимает кнопку "Продолжить"
    Then появляется сообщение неверном формате данных владельца
    Examples:
    | locale |
  ...
  Scenario: Отправка формы со значением поля "CVC/CVV", состоящим из трех одинаковых цифр
    When пользователь заполняет форму, указав три одинаковых цифры в поле "CVC/CVV"
    And нажимает кнопку "Продолжить"
    Then появляется сообщение о неверном формате данных кода
  ...
  Scenario: Отправка формы со значением поля "CVC/CVV", состоящим из четырех цифр
    When пользователь заполняет форму, указав в поле "CVC/CVV" <x> цифр
